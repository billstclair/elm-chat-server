port module ArchmageServer exposing (..)

import Archmage.Server.Error exposing (ServerError(..), errnum)
import Archmage.Server.Interface
    exposing
        ( emptyServerState
        , errorRsp
...
import ArchmageServer.EncodeDecode exposing (decodeMessage, encodeMessage)
import Char
...
socketMessage model socket request =
    case decodeMessage request of
        ...
        Ok message ->
            let
                ( state, response ) =
                    processServerMessage model.state message
            in
            ...
            case messageGameid message of
                ...
                Just gameid ->
                    let
                        ...
                        newNames =
                            case Dict.get gameid model.namesDict of
                                Nothing ->
                                    names
